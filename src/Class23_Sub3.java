...
{
	private static int anInt2069;
	private float[] aFloatArray2070;
	private static Class37[] aClass37Array2071;
	private int anInt2072;
	private byte[][] aByteArrayArray2073;
	private static float[] aFloatArray2074;
	private static Class22[] aClass22Array2075;
	private static float[] aFloatArray2076;
	private static int[] anIntArray2077;
	private static int[] anIntArray2078;
	private static float[] aFloatArray2079;
	private boolean aBoolean2080;
	private static int[] anIntArray2081;
	private static int anInt2082;
	private static int anInt2083;
	private static float[] aFloatArray2084;
	static Class77[] aClass77Array2085;
	private int anInt2086;
	private int anInt2087;
	private static boolean aBoolean2088 = false;
	private static float[] aFloatArray2089;
	private static byte[] aByteArray2090;
	private static float[] aFloatArray2091;
	private static int anInt2092;
	private static float[] aFloatArray2093;
	private static boolean[] aBooleanArray2094;
	private int anInt2095;
	private static Class101[] aClass101Array2096;
	private int anInt2097;
	private int anInt2098;
	private boolean aBoolean2099;
	private int anInt2100;
	private int anInt2101;
	private byte[] aByteArray2102;
	
	private static final boolean method242(Class105 class105) {
		if (!aBoolean2088) {
			byte[] bs = class105.method1544(0, 0, 0);
			if (bs == null) {
				return false;
			}
			method248(bs);
	...
	private static final void method243(byte[] bs, int i) {
		aByteArray2090 = bs;
		anInt2092 = i;
		anInt2082 = 0;
	}
	...
	private final float[] method245(int i) {
		...
		if (bool_13_) {
			...
			for (int i_69_ = i_5_; i_69_ < i_6_; i_69_++) {
				float f = (float) Math.sin(((double) (i_69_ - i_5_) + 0.5) / (double) i_7_ * 0.5 * 3.141592653589793);
				aFloatArray2084[i_69_] *= (float) Math.sin(1.5707963267948966 * (double) f * (double) f);
			}
			for (int i_70_ = i_8_; i_70_ < i_9_; i_70_++) {
				float f = (float) Math.sin(((double) (i_70_ - i_8_) + 0.5) / (double) i_10_ * 0.5 * 3.141592653589793 + 1.5707963267948966);
				aFloatArray2084[i_70_] *= (float) Math.sin(1.5707963267948966 * (double) f * (double) f);
			}
		}
		float[] fs = null;
		if (anInt2095 > 0) {
	...
	static final Class23_Sub3 method246(Class105 class105, int i, int i_77_) {
		if (!method242(class105)) {
			class105.method1562(-14250, i, i_77_);
			return null;
		}
		byte[] bs = class105.method1544(i_77_, 0, i);
		if (bs == null) {
			return null;
		}
	...
	final Class23_Sub6_Sub1 method247(int[] is) {
		if (is != null && is[0] <= 0) {
			return null;
		}
		...
		aByteArray2102 = null;
		return new Class23_Sub6_Sub1(anInt2072, bs, anInt2098, anInt2087, aBoolean2099);
	}
	
	private static final void method248(byte[] bs) {
		method243(bs, 0);
		...
		for (int i = 0; i < 2; i++) {
			...
			for (int i_85_ = 0; i_85_ < i_83_; i_85_++) {
				fs[2 * i_85_] = (float) Math.cos((double) (4 * i_85_) * 3.141592653589793 / (double) i_81_);
				fs[2 * i_85_ + 1] = -(float) Math.sin((double) (4 * i_85_) * 3.141592653589793 / (double) i_81_);
			}
			float[] fs_86_ = new float[i_82_];
			for (int i_87_ = 0; i_87_ < i_83_; i_87_++) {
				fs_86_[2 * i_87_] = (float) Math.cos((double) (2 * i_87_ + 1) * 3.141592653589793 / (double) (2 * i_81_));
				fs_86_[2 * i_87_ + 1] = (float) Math.sin((double) (2 * i_87_ + 1) * 3.141592653589793 / (double) (2 * i_81_));
			}
			float[] fs_88_ = new float[i_83_];
			for (int i_89_ = 0; i_89_ < i_84_; i_89_++) {
				fs_88_[2 * i_89_] = (float) Math.cos((double) (4 * i_89_ + 2) * 3.141592653589793 / (double) i_81_);
				fs_88_[2 * i_89_ + 1] = -(float) Math.sin((double) (4 * i_89_ + 2) * 3.141592653589793 / (double) i_81_);
			}
			int[] is = new int[i_84_];
			int i_90_ = Class100.method1504(8, i_84_ - 1);
			for (int i_91_ = 0; i_91_ < i_84_; i_91_++)
				is[i_91_] = Class40.method1106(i_90_, i_91_, 32768);
			if (i != 0) {
				aFloatArray2093 = fs;
	...
	static final float method249(int i) {
		...
		return (float) ((double) i_103_ * Math.pow(2.0, (double) (i_105_ - 788)));
	}
